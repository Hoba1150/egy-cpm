...
import { Gift, Clock, Trophy, User, Phone, Gamepad2, CheckCircle2, AlertCircle } from "lucide-react";
    ...
    <div className="space-y-6 text-right">
      {giveaways.length === 0 ? (
        <div className="py-16 text-center rounded-2xl bg-[#12161f] border border-gray-800 space-y-3">
          ...
          <h3 className="text-base font-bold text-white">لا توجد سحوبات نشطة حالياً</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {giveaways.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className={`rounded-2xl border p-4 sm:p-5 flex flex-col justify-between space-y-4 transition ${
                  isWinnerAnnounced
                    ? "bg-[#121a14] border-green-500/40 shadow-sm"
                    : "bg-[#12161f] border-gray-800 hover:border-orange-500/40"
                }`}
              >
                <div className="space-y-3">
                  {/* Top Status & Winner Header */}
                  <div className="flex items-center justify-between border-b border-gray-800/80 pb-2.5">
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-black/50 text-gray-300">
                      {g._count?.entries || 0} مشترك
                    </span>

                    {isWinnerAnnounced ? (
                      <span className="px-2.5 py-0.5 rounded bg-green-500/20 text-green-400 font-bold text-xs flex items-center gap-1">
                        <Trophy className="w-3.5 h-3.5" />
                        <span>تم إعلان الفائز</span>
                      </span>
                    ) : (
                      <span className="px-2.5 py-0.5 rounded bg-orange-500/20 text-orange-400 font-bold text-xs flex items-center gap-1">
                  ...
                  {isWinnerAnnounced && (
                    <div className="p-3 rounded-xl bg-green-950/40 border border-green-500/50 text-center space-y-1">
                      <span className="text-[11px] text-green-300 block font-bold">الفائز المحظوظ بالسحب</span>
                      <h4 className="text-base font-black text-white">
                        مبروك للأخ: <span className="text-green-400">{g.winnerName}</span>
                      </h4>
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-orange-400 font-bold mt-0.5">الجائزة: {g.prizeName}</p>
                ...
                <div className="pt-3 border-t border-gray-800/80 flex items-center justify-between">
                  <div className="text-right">
                    <span className="text-[10px] text-gray-400 block">رسوم الاشتراك:</span>
                  ...
                  {!isFinished ? (
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-black font-extrabold text-xs transition"
                    >
                      اشترك في السحب الآن
                    </button>
                  ) : (
      ...
      {selectedGiveaway && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="max-w-md w-full rounded-2xl bg-[#12161f] border border-orange-500/40 p-5 text-right space-y-4">
            <div className="border-b border-gray-800 pb-2.5">
              <h3 className="text-base font-black text-white">التسجيل في {selectedGiveaway.title}</h3>
              <p className="text-xs text-orange-400">الجائزة: {selectedGiveaway.prizeName}</p>
            </div>

            <form onSubmit={handleJoin} className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">الاسم الكريم *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">حسابك في اللعبة (Game ID / Username) *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 bg-[#1a202c] border border-gray-700 rounded-lg text-xs text-white text-right dir-ltr"
                />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">رقم الواتساب للتواصل والتسليم *</label>
                <input
                  type="tel"
                  required
              ...
              {selectedGiveaway.entryFee > 0 && (
                <div className="p-2.5 rounded-lg bg-orange-500/10 border border-orange-500/30 text-[11px] text-orange-300">
              ...
              <div className="flex items-center gap-2 pt-2">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-black font-extrabold text-xs transition"
                >
