...
export default async function WalletPage() {
  ...
  const getTransactionBadge = (type: string) => {
    switch (type) {
      case "DEPOSIT":
        return <span className="px-2.5 py-1 rounded-lg bg-green-500/10 text-neon-green border border-green-500/30 text-xs font-bold">إيداع رصيد +</span>;
      case "PURCHASE":
        return <span className="px-2.5 py-1 rounded-lg bg-cyan-500/10 text-neon-cyan border border-cyan-500/30 text-xs font-bold">شراء طلب -</span>;
      case "GIFT":
        return <span className="px-2.5 py-1 rounded-lg bg-purple-500/10 text-neon-purple border border-purple-500/30 text-xs font-bold">رصيد هدية 🎁</span>;
      case "REFUND":
        return <span className="px-2.5 py-1 rounded-lg bg-amber-500/10 text-neon-amber border border-amber-500/30 text-xs font-bold">استرجاع مالي 💰</span>;
      case "MANUAL_CREDIT":
        return <span className="px-2.5 py-1 rounded-lg bg-blue-500/10 text-neon-blue border border-blue-500/30 text-xs font-bold">إضافة يدوية +</span>;
      default:
        return <span className="px-2.5 py-1 rounded-lg bg-red-500/10 text-neon-red border border-red-500/30 text-xs font-bold">خصم يدوي -</span>;
    }
  };
  ...
  const getDepositStatusBadge = (status: string) => {
    switch (status) {
      case "APPROVED":
        return (
          <span className="inline-flex items-center gap-1 text-neon-green text-xs font-bold bg-green-500/10 px-2 py-0.5 rounded-md border border-green-500/30">
      ...
      case "REJECTED":
        return (
          <span className="inline-flex items-center gap-1 text-neon-red text-xs font-bold bg-red-500/10 px-2 py-0.5 rounded-md border border-red-500/30">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 text-neon-amber text-xs font-bold bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/30 animate-pulse">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-right space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-800 pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-4xl font-black text-white">
            المحفظة وسجل المعاملات المالية
          </h1>
        </div>

        <Link
          href="/deposit"
          className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-neon-green to-emerald-500 text-black font-extrabold text-xs sm:text-sm shadow-glow-green hover:scale-105 transition flex items-center justify-center gap-2 self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="p-6 rounded-3xl bg-garage-900 border border-cyan-500/40 shadow-glow-cyan-sm space-y-2 relative overflow-hidden">
          ...
          <p className="text-[11px] text-neon-green">جاهز للشراء الفوري لكافة الخدمات</p>
        </div>

        {/* Regular Balance */}
        <div className="p-6 rounded-3xl bg-garage-900 border border-gray-800 space-y-2">
          <div className="flex items-center justify-between text-gray-400 text-xs">
            <span>الرصيد المشحون</span>
            <ArrowDownLeft className="w-4 h-4 text-neon-green" />
          </div>
          <h3 className="text-2xl sm:text-3xl font-black text-neon-green font-mono">
        ...
        {/* Gift Balance */}
        <div className="p-6 rounded-3xl bg-garage-900 border border-purple-500/30 shadow-glow-purple space-y-2">
          <div className="flex items-center justify-between text-gray-400 text-xs">
            <span>رصيد الهدايا المجاني</span>
            <Gift className="w-4 h-4 text-neon-purple" />
          </div>
          <h3 className="text-2xl sm:text-3xl font-black text-neon-purple font-mono">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right text-xs">
              ...
              <tbody className="divide-y divide-gray-800/60">
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-garage-850/50 transition">
                    ...
                    <td className="py-3.5 text-neon-cyan font-mono font-bold">
