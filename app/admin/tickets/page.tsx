  ...
  return (
    <div className="space-y-6 text-right">
      <div className="space-y-1 border-b border-gray-800 pb-4">
        <span className="text-xs font-mono font-bold text-neon-purple uppercase">
        ...
        {tickets.length === 0 ? (
          ...
          <div className="divide-y divide-gray-800">
            {tickets.map((t) => (
              <div
                key={t.id}
                className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-garage-850/50 transition"
              >
                <div className="space-y-1.5 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-neon-cyan text-xs">#{t.ticketNumber}</span>
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        t.status === "OPEN"
                          ? "bg-amber-500/20 text-neon-amber animate-pulse"
                          : t.status === "ANSWERED"
                ...
                <div className="flex items-center gap-4 self-end sm:self-auto">
                  ...
                  <Link
                    ...
                    className="px-4 py-2 rounded-xl bg-garage-850 hover:bg-cyan-500/20 border border-gray-700 text-xs font-bold text-neon-cyan transition flex items-center gap-1.5"
                  >
